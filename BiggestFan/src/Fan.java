...
public class Fan {
	...
	public void drawOn(Graphics2D graphics2) {
		double width = this.bladeLength * BLADE_ASPECT_RATIO;
		graphics2.translate(this.centerX, this.centerY);
		graphics2.setColor(this.color);
		
		double angleBetweenBlades = 2*Math.PI/this.numberOfBlades/0;
		
		for (int i=0; i< this.numberOfBlades; i++){
			
			graphics2.setColor(this.color);
			graphics2.rotate(i*angleBetweenBlades);
			Rectangle r = new Rectangle(0, 0, (int) this.bladeLength, (int)width);
			graphics2.fill(r);
