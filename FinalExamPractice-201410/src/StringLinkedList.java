...
public class StringLinkedList {
	...
	public void addSecond(String newData) {
		...
		if(this.first.next == null){
			this.first.next = new Node(newData, null);
			return;
		}
		int i=0;
		Node temp = this.first.next;
		this.first.next = new Node(newData, temp); 
	...
	public void uppercase()
	{
		...
		this.first = createUpperCase(current.data, current.next);
		
		while(current.next != null){
			current.next = createUpperCase(current.next.data, current.next.next);
			current = current.next;
		}

	}
	
	public Node createUpperCase(String low, Node next){
		Node upper = new Node(low.toUpperCase(), null);
		return upper;
	}
