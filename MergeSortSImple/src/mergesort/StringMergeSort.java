...
public class StringMergeSort {
	...
	public static String stringMergeSort(String input) {
		...
		String finalPart = input.substring(length, input.length());
		String firstRec = stringMergeSort(firstPart);
		String SecondRec = stringMergeSort(finalPart);
		while (firstPart.length()>1 || finalPart.length()>1){
			
			if(firstPart.length()<= 1 && finalPart.length()<=1){
				
				if(firstPart.charAt(0) <= finalPart.charAt(0)){
					return stringMergeSort(firstPart+finalPart);
				}
				if(firstPart.charAt(0) > finalPart.charAt(0)){
					return stringMergeSort(finalPart+firstPart);
				}
			}
		}
